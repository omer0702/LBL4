...
// Parse bytes: try to parse header; returns DecodeResult and sets header/out_payload/out_len when OK
DecodeResult parse_frame_header(const uint8_t* data, size_t data_len, FrameHeader& out_header);

// Utility: append length-prefixed framed message to socket send buffer or return vector
